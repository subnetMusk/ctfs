...
# state conversion (bytes2matrix first)
...
def add_round_key(st, key):
    ...
    for i in range(len(st)):
        p = [] #ho corretto questo per conservare la forma dello state_matrix
        for j in range(len(st[i])):
            p.append(st[i][j] ^ key[i % len(key)][j % len(key[i])])
        r.append(p)
    return r
...
# inverse sub bytes (basta passargli inv_sub_box al posto di sub_box)
...
def decrypt(key, ciphertext):
    ...
    state_matrix = add_round_key(state_matrix, round_keys[N_ROUNDS])
    state_matrix = inv_shift_rows(state_matrix)
    state_matrix = sub_bytes(state_matrix, sbox=inv_s_box)
    ...
    return matrix2bytes(state_matrix)
